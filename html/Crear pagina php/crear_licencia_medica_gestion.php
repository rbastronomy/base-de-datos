...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $sql_licencia = "INSERT INTO licencia_medica (id_ficha, rut, emp_rut, lugar_reposo, centro_medico, f_otorgamiento, f_inicio_reposo, f_termino_reposo, diagnostico, medico)
                        VALUES (:id_ficha, :rut, :emp_rut, :lugar_reposo, :centro_medico, :f_otorgamiento, :f_inicio_reposo, :f_termino_reposo, :diagnostico, :medico)";

        $stmt_licencia = $conexion->prepare($sql_licencia);
        $stmt_licencia->bindParam(':id_ficha', $id_ficha);
        $stmt_licencia->bindParam(':rut', $rut);
        $stmt_licencia->bindParam(':emp_rut', $emp_rut);
...
// Obtener las ID de ficha y RUT de la tabla "ficha_clinica" que sean empleados
try {
    $host = 'magallanes.inf.unap.cl';
    $port = '5432';
    $dbname = 'jgomez';
    ...
    $stmt_fichas = $conexion->query("SELECT f.id_ficha, e.rut, e.nombre
                                    FROM ficha_clinica f
                                    INNER JOIN empleado e ON f.rut = e.rut");
...
<head>
    ...
    <script>
        function fillEmpRut() {
            var idFicha = document.getElementById("id_ficha").value;
            var empRutField = document.getElementById("emp_rut");

            // Buscar el RUT del empleado correspondiente a la ficha seleccionada
            var selectedEmpRut = "<?php foreach ($fichas as $ficha) {
                                    if ($ficha['id_ficha'] == '"+idFicha+"') {
                                        echo $ficha['rut'];
                                    }
                                } ?>";

            empRutField.value = selectedEmpRut;
        }
    </script>
</head>
<style>
    ...
    <div class="container">
        ...
        <form action="crear_licencia_medica.php" method="POST" class="paciente-form">
            <div class="form-row">
                ...
                <select id="id_ficha" name="id_ficha" required onchange="fillEmpRut()">
                    <?php foreach ($fichas as $ficha) { ?>
                        <option value="<?php echo $ficha['id_ficha']; ?>"><?php echo $ficha['id_ficha']; ?></option>
                    <?php } ?>
                </select>
            </div>
            ...
            <div class="form-row">
                ...
                <input type="text" id="rut" name="rut" value="<?php echo $rut; ?>" required>
            </div>

            <div class="form-row">
                <label for="emp_rut">RUT Empleado:</label>
                <input type="text" id="emp_rut" name="emp_rut" required>
            </div>
